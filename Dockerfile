...
COPY constants.py utils.py data.py charts.py dashboard_armoedebeleid.py ./
...
RUN python -m compileall constants.py utils.py data.py charts.py dashboard_armoedebeleid.py

# Note: Parquet file loaded from Dropbox URL at runtime (not bundled in container)
...
# Note: XSRF protection is disabled because Cloud Run handles authentication
# externally and the internal request path does not include XSRF tokens.
